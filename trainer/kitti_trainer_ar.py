...
class TrainFramework(BaseTrainer):
    ...
    def _run_one_epoch(self):
        ...
        if "stage2" in self.cfg and self.i_epoch >= self.cfg.stage2.epoch:
            ...
            if self.cfg.key_obj_aug:
                self.set_up_obj_cache(cache_size=100)  # 500)
            self.cfg.pop("stage2")
        ...
        while i_step < self.cfg.epoch_size:
            for data in self.train_loaders[self.i_train_set]:
                ...
                # Remove segmentation-related data loading
                ...
                ## DEBUG: gradient of each loss
                # import IPython; IPython.embed(); exit()
                # import matplotlib.pyplot as pl
                # self.zero_grad()
                # flows[0].grad = None
                # flows[0].retain_grad()
                # l_ph.backward(retain_graph=True)
                # l_sm.backward(retain_graph=True)
                # grad = flows[0].grad[:, :2, :, :].abs().sum(axis=1).cpu().numpy().reshape((-1, flows[0].shape[-1]))
                # print("max = {}, median = {}".format(grad.max(), np.median(grad)))
                # plt.imsave("_DEBUG_GRADIENT.png", np.clip(grad / grad.max() * 10, 0, 1))
    ...
    def _validate_with_gt(self):
        ...
        for i_set, loader in enumerate(self.valid_loaders):
            ...
            flow_error_names = ["EPE_all", "EPE_noc", "EPE_occ", "Fl_all", "Fl_noc"]
            ...
            for i_step, data in enumerate(loader):
                ...
                gt_flows = [
                    ...
                    for flow, occ_mask, noc_mask in zip(gt_flows, occ_masks, noc_masks)
                ]
